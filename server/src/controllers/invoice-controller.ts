...
import { createInvoice, deleteInvoice, getAllInvoicesForBusiness, getInvoiceDetails } from "../service/invoices";
...
const handleUpdateInvoices = async (req: Request, res: Response) => {
    ...
    try {
        res.status(HttpStatusCode.ACCEPTED).json(ResponseEntity("success", "Invoices Updated Successfully!"))
    } catch (error) {
        const message = (error as Error).message;
